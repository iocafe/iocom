/**
  ...
  @file    tealight.c
  @brief   Tealight camera IO example.
 ...
 */
static iocStreamerParams ioc_ctrl_stream_params = IOBOARD_DEFAULT_CTRL_STREAM(tealight,
    ...
    os_char *argv[])
{
     ...
     */
    DINFO_SET_COMMON_NET_CONF_SIGNALS_FOR_WIFI(nc_sigs, tealight);
    DINFO_SET_COMMON_RESOURCE_MONITOR_SIGNALS(rm_sigs, tealight);
    ...
    prm.exp_signal_hdr = &tealight.exp.hdr;
    prm.imp_signal_hdr = &tealight.imp.hdr;
    prm.dexp_signal_hdr = &tealight.dexp.hdr;
    prm.dimp_signal_hdr = &tealight.dimp.hdr;
    prm.conf_exp_signal_hdr = &tealight.conf_exp.hdr;
    prm.conf_imp_signal_hdr = &tealight.conf_imp.hdr;
     ...
     */
    ioc_initialize_tealight_parameters(&tealight, OS_PBNR_CUST_A, OS_NULL);
    ioc_load_tealight_parameters(&tealight);

    /* Set up device information.
     */
    dinfo_set_node_conf(&dinfo_nc, device_id, connconf, nics, wifis, security);
    DINFO_SET_COMMON_SYSTEM_SPECS_SIGNALS(si_sigs, tealight);
    dinfo_set_system_specs(&si_sigs, CANDY_HW);

    /* Set callback to pass communcation to pins.
     */
    ioc_add_callback(&ioboard_imp, ioboard_communication_callback, (void*)&tealight_hdr);
    ...
    ioc_initialize_brick_buffer(&video_output, &tealight.camera,
    ...
    morse.blink_level[0] = (os_short)ioc_get(&tealight.exp.hlight_blink);
    morse.steady_hdlight_level[0] = (os_short)ioc_get(&tealight.exp.hlight_lvl);
#endif
    ...
    void *app_context)
{
     ...
     */
    ioc_autosave_tealight_parameters(&tealight);
...
#if IOCOM_USE_MORSE==2
            if (sig == &tealight.imp.set_hlight_lvl) {
                morse.steady_hdlight_level[0] = (os_short)ioc_get(sig);
            }
            if (sig == &tealight.imp.set_hlight_blink) {
...
static void ioboard_configure_camera(void)
{
    ...
    ioboard_set_camera_prm(PINS_CAM_NR, &tealight.exp.cam_nr);
#endif
#ifdef CANDY_EXP_IMG_WIDTH
    ioboard_set_camera_prm(PINS_CAM_IMG_WIDTH, &tealight.exp.img_width);
    ioboard_get_camera_prm(PINS_CAM_IMG_WIDTH, &tealight.exp.img_width);
    ioboard_get_camera_prm(PINS_CAM_IMG_HEIGHT, &tealight.exp.img_height);
#endif
#ifdef CANDY_EXP_IMG_HEIGHT
    ioboard_set_camera_prm(PINS_CAM_IMG_HEIGHT, &tealight.exp.img_height);
    ioboard_get_camera_prm(PINS_CAM_IMG_WIDTH, &tealight.exp.img_width);
    ioboard_get_camera_prm(PINS_CAM_IMG_HEIGHT, &tealight.exp.img_height);
#endif
#ifdef CANDY_EXP_FRAMERATE
    ioboard_set_camera_prm(PINS_CAM_FRAMERATE, &tealight.exp.framerate);
#endif

#ifdef CANDY_EXP_BRIGHTNESS
    ioboard_set_camera_prm(PINS_CAM_BRIGHTNESS, &tealight.exp.brightness);
#endif
#ifdef CANDY_EXP_SATURATION
    ioboard_set_camera_prm(PINS_CAM_SATURATION, &tealight.exp.saturation);
#endif
}
...
static void ioapp_turn_camera_on_or_off(void)
{
    ...
    turn_on = (os_boolean)ioc_get(&tealight.exp.on);
