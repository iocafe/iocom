#!/usr/bin/env python3
# copy_deviceinfo_for_duino.py 15.7.2020/pekka
# Copies deviceinfo library files needed for PlatformIO Arduino build
# into /coderoot/lib/arduino-platformio/deviceinfo directory.
...
def mymain():
    outdir = "/coderoot/lib/arduino-platformio/deviceinfo"
    expectplatform = True
    n = len(sys.argv)
    for i in range(1, n):
        if sys.argv[i][0] == "-":
    ...
    copy_level_1("/coderoot/iocom/extensions/deviceinfo", outdir)

# Usage copy_deviceinfo_for_duino.py -o /coderoot/lib/esp32/deviceinfo
mymain()
